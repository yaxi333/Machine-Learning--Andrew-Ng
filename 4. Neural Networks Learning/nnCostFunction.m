...
for i = 1:m
    ...
    % Transfer the label vector to binary form(Logical arrays)
    y_i = zeros(size(h,1),1); y_i_idx = y(i); y_i(y_i_idx) = 1;
    
    % Calculate the cost J(theta) for ith example
    J_i = (- y_i' * log(h) - (1-y_i)' * log(1-h))/ m;
    J = J + J_i;   
    
    %  Accumulated gradients
    deta3 = h - y_i;
    deta2 = (Theta2' * deta3) .* [1;sigmoidGradient(z2)];
    deta2 = deta2(2:end);
